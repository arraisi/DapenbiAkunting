...
jQuery(document).ready(function () {
    ...
    $('#txtArusKas').change(function () {
        $(this).prop('checked') === true ? showWarning('Transaksi ini akan dihitung pada Laporan Arus Kas')
            : showWarning('Transaksi ini tidak akan dimasukkan ke Laporan Arus Kas');
    });
});
...
function updateStatusWarkat(status) {
    ...
    if (status === 'REJECT') {
        if (_currentWarkat.catatanValidasi === '') {
            showWarning("Catatan tidak boleh kosong");
            return false;
        }
    }
    ...
    // console.log(_currentWarkat);

    swal.fire({
        title: "Are you sure?",
        text: "Mohon perhatikan lagi kolom arus kas.",
        type: "warning",
        ...
        if (e.value) {
            ...
            Pace.track(function () {
                $.ajax({
                    ...
                    url: _baseUrl + `/akunting/transaksi/saldo/validasi/final-approval`,
...
function warkatDatatables() {
    ...
    $('#tblWarkat tbody').on('click', '.validation-button', function () {
        ...
        _currentWarkat = tableWarkat.row($(this).parents('tr')).data();
        // console.log(data);
