...
function jurnalBiayaDatatable() {
    ...
    // BUTTON DELETE
    $('#jurnalBiayaDatatable tbody').on('click', '.delete-button', function () {
    ...
    // BUTTON EDIT
    $('#jurnalBiayaDatatable tbody').on('click', '.edit-button', function () {
        var data = _jurnalBiayaDatatable.row($(this).parents('tr')).data();
        console.log(data);
...
function tambahDataBtnPressed() {
    resetFormJurnalBiaya();
    hideFormJurnalBtnPressed();
    resetTableJurnal();
    $('#tambahJurnalBtn').attr('disabled', true);
    $('#tampilkanDataBtn').attr('disabled', false);
    $(".rowWarkat").attr("hidden", true);
    $('#newJurnalBiayaDialog').modal('show');
}

function findJurnalBiayaDebits() {
    $.ajax({
        type: "GET",
        contentType: "application/json",
        url: _baseUrl + "/akunting/transaksi/jurnal-biaya/findDebits",
...
function displayTambahJurnalForm(data) {
    ...
    lastNoUrut === "Tidak ada data" ? $('#txtUrutanTambahJurnal').val(1) : $('#txtUrutanTambahJurnal').val((parseInt(lastNoUrut) + 1));
...
function fatchFormJurnalBiaya(data) {
...
function saveJurnalBiaya(status) {

    var request = {
        tglTransaksi: $("#txtTglTransaksi").val(),
        tglBuku: $("#txtTglBuku").val(),
        kodeTransaksi: {kodeTransaksi: 'JBIAYA'},
    ...
    if (!$('#tambahJurnalForm').is(':visible')) {
        serviceSaveJurnalBiaya(request);
    } else {
        swal.fire({
    ...
    $('#newJurnalBiayaDialog').modal('toggle');
}

function serviceSaveJurnalBiaya(data) {
    // console.log(data);
    $.ajax({
        type: "POST",
        contentType: "application/json",
        data: JSON.stringify(data),
        url: _baseUrl + `/akunting/transaksi/jurnal-biaya/saldo-warkat/and/jurnals`,
        success: function (response) {
