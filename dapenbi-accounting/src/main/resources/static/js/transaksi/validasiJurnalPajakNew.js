var _totalTransaksi;
var _currentJurnalPajak;
var _jurnalPajakDatatable;
var _jurnals = [];
var _totalDebit = 0;
var _totalKredit = 0;
...
jQuery(document).ready(function () {
    jurnalPajakDatatable();
});

function updateStatus(status) {
    _currentJurnalPajak.statusData = status;
    _currentJurnalPajak.keterangan = $('#txtKeterangan').val();
    _currentJurnalPajak.catatanValidasi = $('#catatan').val();

    if (status === 'REJECT') {
        if (_currentJurnalPajak.catatanValidasi === '') {
            showWarning("Keterangan dan Catatan tidak boleh kosong");
            return false;
        }
    }

    $('#jurnalPajakDialog').modal('hide');
    $('#catatanTolakDialog').modal('hide');

    $.ajax({
        type: "POST",
        contentType: "application/json",
        data: JSON.stringify(_currentJurnalPajak),
...
function jurnalPajakDatatable() {
    ...
    // BUTTON EDIT
    $('#jurnalPajakDatatable tbody').on('click', '.validation-button', function () {

        var data = _jurnalPajakDatatable.row($(this).parents('tr')).data();
        // console.log(data);
        _currentJurnalPajak = data;
        $("#txtJenisTransaksi").val(_currentJurnalPajak.kodeTransaksi.kodeTransaksi);
        $("#txtNoWarkat").val(_currentJurnalPajak.noWarkat);
        $("#txtTglTransaksi").val(moment(_currentJurnalPajak.tglBuku).format('yyyy-MM-DD'));
        $("#txtTglBuku").val(moment(_currentJurnalPajak.tglBuku).format('yyyy-MM-DD'));
        $("#txtTahunBuku").val(_currentJurnalPajak.tahunBuku);
        _totalTransaksi = _currentJurnalPajak.totalTransaksi;
        $("#txtTotalTransaksi").val(numeralformat(_totalTransaksi));
        $("#txtKeterangan").val(_currentJurnalPajak.keterangan);
        $("#txtTglInput").val(moment(_currentJurnalPajak.createdDate).format('yyyy-MM-DD'));
        $("#txtUserInput").val(_currentJurnalPajak.createdBy);
...
function findJurnals() {
    $.ajax({
        type: "GET",
        contentType: "application/json",
        url: _baseUrl + "/api/akunting/warkat-jurnal/findByNoWarkat?noWarkat=" + _currentJurnalPajak.noWarkat,
        success: function (response) {
