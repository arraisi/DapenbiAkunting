...
jQuery(document).ready(function () {
    var jenisDRI = message == 1 ? "DRI Harian" : message == 2 ? "DRI Pajak" : message == 3 ? "DRI Final" : "Not Found";
    $("#cardTitle").text(jenisDRI);
    ...
    table = $("#tblPeriode").DataTable({
        ajax: {
            ...
            url: _baseUrl + '/akunting/dri/getDRIDatatables',
            type: "POST",
            data: function (d) {
                var dri = {};
                var order  = [];
                ...
                dri.kodeDRI = message;
                dri.tglPeriode = convertStringDate($("#tglPeriode").val());
                dri.idRekening = null;
                dri.kodeRekening = $("#idRekening").val();
                d.dri = dri;
                d.order = order;
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (data) {
                    ...
                    if (data.levelRekeningDebit != '1' && !data.namaRekeningDebit.includes("TOTAL DEBIT")) {
...
//    return;
    ...
    $.ajax({
        ...
        url: _baseUrl + "/akunting/dri/export-pdf-dri?periode=" + periode + "&kodeDRI=" + kodeDRI + "&ttd=" + ttd + "&jenisDRI=" + jenisDRI + "&tanggal=" + tanggal,
        success: function (response) {
            var link = document.createElement("a");
            link.href = "data:application/pdf;base64, " + response;
            link.download = "DRI "+message+".pdf";
            link.target = "_blank";
...
function exportExcel(table) {
    ...
    $.ajax({
        ...
        contentType: "application/json",
        data: JSON.stringify(obj),
        url: _baseUrl + "/akunting/dri/export-excel-dri?periode=" + periode + "&kodeDRI=" + kodeDRI + "&ttd=" + ttd + "&jenisDRI=" + jenisDRI + "&tanggal=" + tanggal,
        success: function (response) {
            var link = document.createElement("a");
            link.href = "data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64, " + response;
            link.download = "DRI "+message+".xlsx";
            link.target = "_blank";
...
//        data[i].saldoAkhir = data[i].saldoAkhir == null ? "" : formatMoney(data[i].saldoAkhir.toString());
        rows.push(data[i]);
