...
jQuery(document).ready(function () {
    ...
    table = $("#tblSPI").DataTable({
        ajax: {
            contentType: "application/json",
            url: _baseUrl + "/akunting/laporan/laporan-spi/datatables",
            type: "POST",
            data: function (d) {
                var obj = {
                    "tglTransaksi": "",
...
function initEvent() {
    $("#export-pdf").click(function () {
        exportPdf($('#tblSPI').DataTable());
    });
    $("#export-excel").click(function () {
        exportExcel($('#tblSPI').DataTable());
    });
}
...
function exportPdf(table) {
    ...
    $.ajax({
        ...
        url: _baseUrl + "/akunting/laporan/laporan-spi/export-pdf?tanggal=" + tanggal,
        success: function (response) {
            var link = document.createElement("a");
            link.href = "data:application/pdf;base64, " + response;
            link.download = "Laporan SPI.pdf";
            link.target = "_blank";
