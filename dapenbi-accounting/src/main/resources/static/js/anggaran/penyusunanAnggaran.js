...
jQuery(document).ready(function () {
    ...
    $("#btnDownload").hide();
    $("#dataExcel").hide();
    $("#btnSubmitAnggaran").hide();

    var table = $("#tblAnggaran").DataTable({
        ajax: {
            contentType: 'application/json',
            url: _baseUrl + '/akunting/anggaran/penyusunan-anggaran/datatables',
            type: "POST",
            data: function (d) {
...
function updateIsEditRekening(status, table) {
    $.ajax({
        type: "PUT",
        contentType: "application/json",
        url: _baseUrl + "/akunting/parameter/rekening/updateEdited?idRekening=" + rekeningSelected.idRekening + "&status=" + status,
        success: function (resp) {
            console.log(resp, "rekening update");
        },
        statusCode: {
            201: function () {
                createExcelFile(table);
...
function initEvent(table) {
    $("#btnSimpanAnggaran").click(function () {
        ...
        if (isValid()) {
            ...
            disabledButton(true);
            $("#txtAnggaranZZ").val(setMoneyFormat(hot.getData()[9][1]));
...
//    return;
    if (mataAnggaran == "") {
        ...
        $.ajax({
            type: "GET",
            contentType: "application/json",
            url: _baseUrl + "/akunting/anggaran/penyusunan-anggaran/data-relasi?mataAnggaran=" + mataAnggaran + "&kodeTahunBuku=" + kodeTahunBuku,
            success: function (resp) {
                console.log(resp, "data realisasi");
                $("#txtRealisasi").val(setMoneyFormat(resp));
                hot.setDataAtCell(6, 1, $("#txtRealisasi").val());
            },
            statusCode: {
                500: function (resp) {
                    console.log(resp.responseJSON.message);
...
function getDataAT(hot) {
    ...
    if (mataAnggaran == "") {
        ...
        $.ajax({
            type: "GET",
            contentType: "application/json",
            url: _baseUrl + "/akunting/anggaran/penyusunan-anggaran/data-at?mataAnggaran=" + mataAnggaran + "&kodeTahunBuku=" + kodeTahunBuku,
            success: function (resp) {
                $("#txtAnggaranXX").val(setMoneyFormat(resp));
                hot.setDataAtCell(8, 1, $("#txtAnggaranXX").val());
            },
            statusCode: {
                500: function (resp) {
                    console.log(resp.responseJSON.message);
...
function update(table, statusData) {
    ...
    $.ajax({
        ...
        url: _baseUrl + "/akunting/anggaran/penyusunan-anggaran/update?statusData=" + statusData,
...
function deleteById(table) {
    ...
    $.ajax({
        type: "POST",
        contentType: "application/json",
        url: _baseUrl + "/akunting/anggaran/penyusunan-anggaran/delete/" + noAnggaran,
        success: function (response) {
...
function setAnggaranDialogValue(data) {
    ...
    $("#satker").val(data.idSatker.kodeLookup);
}
...
function save(table, statusData) {
    ...
    if (isValid()) {
        $.ajax({
            ...
            contentType: "application/json",
            data: JSON.stringify(obj),
            url: _baseUrl + "/akunting/anggaran/penyusunan-anggaran/save?statusData=" + statusData,
            success: function (response) {
                $('#newAnggaranDialog').modal('hide');
                showSuccess();
                table.ajax.reload();
...
function createExcelFile() {
    ...
    $('#txtMataAnggaran').val(rekeningSelected.kodeRekening + ' - ' + rekeningSelected.namaRekening);
    $('#txtIdMataAnggaran').val(rekeningSelected.idRekening);
    ...
    $.ajax({
        url: _baseUrl + '/akunting/anggaran/penyusunan-anggaran/create-excel/'
...
//
//     $.ajax({
//         url: _baseUrl + '/akunting/anggaran/penyusunan-anggaran/upload',
//         data: data,
//         cache: false,
//         contentType: false,
//         processData: false,
...
function reupload(data) {
    ...
    $.ajax({
        ...
        contentType: "application/json",
        data: JSON.stringify(obj),
        url: _baseUrl + '/akunting/anggaran/penyusunan-anggaran/reupload',
        success: function (resp) {
            showSuccess();
        },
        statusCode: {
...
function findAnggaranByIdRekening(data, table, action) {
    ...
    return $.ajax({
        type: "GET",
        contentType: "application/json",
        url: _baseUrl + `/akunting/anggaran/penyusunan-anggaran/findByIdRekening/${rekeningSelected.idRekening}/satker/${$('#satker').val()}`,
        success: function (resp) {
            // console.log(resp);
        }, statusCode: {
            200: function (resp) {
                ...
                if (statusAksi === "edit" || resp.statusData === 'APPROVE') {
                    ...
                    showWarning(`Mata Anggaran : ${resp.idRekening.namaRekening} sudah ada!`);
                    simpanBtnPressed(false);
                    submitBtnPressed(false);
                    disabledButton(false);
                }
...
function showExcel(resp) {
    ...
    $("#txtPerkiraanRelasi").change(function () {
        hot.setDataAtCell(7, 1, $("#txtPerkiraanRelasi").val());
    });
}
