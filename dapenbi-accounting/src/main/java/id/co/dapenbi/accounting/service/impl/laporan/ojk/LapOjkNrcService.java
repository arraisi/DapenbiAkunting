...
import id.co.dapenbi.accounting.repository.laporan.ojk.OjkNrcRepository;
...
@Service
public class LapOjkNrcService {

    @Autowired
    private OjkNrcRepository repository;

    @Autowired
    private NamedParameterJdbcTemplate namedParameterJdbcTemplate;

    public TableLapOjk<OjkNrc> getDataTable(String kodePeriode, String kodeTahunBuku) {
        DataTablesInput input = new DataTablesInput();
        input.addOrder("idLaporan", true);
        input.setLength(-1);
        Specification<OjkNrc> byKodePeriode = this.byKodePeriode(kodePeriode);
        Specification<OjkNrc> byKodeTahunBuku = this.byKodeTahunBuku(kodeTahunBuku);
        DataTablesOutput<OjkNrc> output = repository.findAll(input, Specification.where(byKodePeriode).and(byKodeTahunBuku));
        Iterable<OjkNrc> ojkLanIterable = output.getData();
    ...
    public List<OjkNrc> findAll() {
       return repository.findAll();
    }

    private Specification<OjkNrc> byKodePeriode(String kodePeriode) {
        return (book, cq, cb) -> cb.equal(book.get("kodePeriode"), kodePeriode);
    }

    private Specification<OjkNrc> byKodeTahunBuku(String kodeTahunBuku) {
        return (book, cq, cb) -> cb.equal(book.get("kodeTahunBuku"), kodeTahunBuku);
    }

    public List<OjkNrc> findAll(String kodePeriode, String kodeTahunBuku) {
        Specification<OjkNrc> byKodePeriode = this.byKodePeriode(kodePeriode);
        Specification<OjkNrc> byKodeTahunBuku = this.byKodeTahunBuku(kodeTahunBuku);
