...
import id.co.dapenbi.accounting.repository.laporan.ojk.OjkPillRepository;
...
@Service
public class LapOjkPillService {

    @Autowired
    private NamedParameterJdbcTemplate namedParameterJdbcTemplate;

    @Autowired
    private OjkPillRepository ojkPillRepository;

    public TableLapOjk<OjkPill> getDatatables(String kodePeriode, String kodeTahunBuku) {
        DataTablesInput input = new DataTablesInput();
        input.addOrder("idLaporan", true);
        input.setLength(-1);
        Specification<OjkPill> byKodePeriode = this.byKodePeriode(kodePeriode);
        Specification<OjkPill> byKodeTahunBuku = this.byKodeTahunBuku(kodeTahunBuku);
        DataTablesOutput<OjkPill> output = ojkPillRepository.findAll(input, Specification.where(byKodePeriode).and(byKodeTahunBuku));
        Iterable<OjkPill> ojkLanIterable = output.getData();
    ...
    public List<TableColumn> getColumns() {
        ...
        columns.add(new TableColumn("Jenis Piutang Lain-lain", "jenisPiutang"));
    ...
    public List<OjkPill> findAll() {
        return ojkPillRepository.findAll();
    }
    ...
    private Specification<OjkPill> byKodePeriode(String kodePeriode) {
        return (book, cq, cb) -> cb.equal(book.get("kodePeriode"), kodePeriode);
    }

    private Specification<OjkPill> byKodeTahunBuku(String kodeTahunBuku) {
        return (book, cq, cb) -> cb.equal(book.get("kodeTahunBuku"), kodeTahunBuku);
    }

    public List<OjkPill> findAll(String kodePeriode, String kodeTahunBuku) {
        Specification<OjkPill> byKodePeriode = this.byKodePeriode(kodePeriode);
        Specification<OjkPill> byKodeTahunBuku = this.byKodeTahunBuku(kodeTahunBuku);
        return ojkPillRepository.findAll(Specification.where(byKodePeriode).and(byKodeTahunBuku));
    }
}
