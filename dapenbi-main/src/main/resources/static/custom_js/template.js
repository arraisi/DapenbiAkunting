...
function checkStatus(){
	$.get(_baseUrl + '/akunting/parameter/pengaturan-sistem/findByStatusAktif', {

	}, function (response) {
        var currentDate = new Date(response.tglTransaksi);
        var date = currentDate.getDate();
        var month = currentDate.getMonth();
        var year = currentDate.getFullYear()
        var monthDateYear  = date + "/" + (month+1) + "/" + year;
		if(response.statusOpen == "O") {
			$('#statusParameter').html('SISTEM DIBUKA');
			$('#tanggalTransaksiParameter').html(monthDateYear);
		} else {
		    $('#statusParameter').html('SISTEM DITUTUP');
		    $('#tanggalTransaksiParameter').html(monthDateYear);
		}
	}).fail(function(){
		$('#modalNoConnection').modal('show');
	...
	$.get(_baseUrl + '/akunting/parameter/pengaturan-sistem/findLatestKodeDRI', {

	}, function (response) {
		$('#latestKodeDRI').html('DRI ' + response);
	}).fail(function(){
		$('#modalNoConnection').modal('show');
	});
}
